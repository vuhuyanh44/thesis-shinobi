...
module.exports = function(__dirname, config){
    ...
    const processImage = async (buffer,d,tx,frameLocation) => {
        ...
        const obj = { cameraId: d.id, message: buffer.toString('base64')};
        // kafkaService.produceBuffer("shinobi-kafka", buffer)
        kafkaService.produce("shinobi-kafka", JSON.stringify(obj))
        // console.log("d: " + d)
        // console.log("tx: " + tx)
        // console.log("frame location: " + frameLocation)
        // kafkaService.produce("shinobi-kafka", buffer.toJson)
        // kafkaService.produce("shinobi-kafka", d.toJson)
        // kafkaService.produce("shinobi-kafka", tx.toJson)
        // kafkaService.produce("shinobi-kafka", frameLocation.toJson)
        s.detectObject(buffer,d,tx,frameLocation,() => {
            --overAllProcessingCount
            sendToParentPort('processCount',overAllProcessingCount)
        })
